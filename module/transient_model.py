...
class PipelineSystem(SysParam):
    ...
    # 11
    def finite_diff(self, format):
        '''

        '''
        # recoder flow and head
        if self.steady_state:
            self.T = self.T + self.time_step
            self.record()
            # if self.T>3:
            #     self.pipes[0].cda[1]=0.2
        # Update internal node flow and head
        for pipe in self.pipes:
            pipe.finite_diff(format)
        ...
        for node in self.nodes:
            ...
            if (node.type == BD.UpperReservoir) or (node.type == BD.LowerReservoir):  # reservoir
                ...
                print('Unknown boundary condition in finite_diff!')
                exit(0)
    ...
    # 12
    def Jocob(self, id_pipeA, id_pipeB, j_A, j_P, j_B):
        pipe_A = self.pipes[id_pipeA]
        pipe_B = self.pipes[id_pipeB]
        Q_A = pipe_A.Q[j_A]
        Q_B = pipe_B.Q[j_B]
        Q_P = pipe_A.QP[j_P]
        K_A = pipe_A.K[j_A]
        K_P = pipe_A.KP[j_P]
        # HA
        k1 = pipe_A.m1 + pipe_A.m2 * abs(Q_A - K_A)
        k2 = -pipe_B.m1 - pipe_B.m2 * abs(Q_B)
        b = 1
        ma, na = self.solve_equations(k1, k2, b)
        # QA
        b = pipe_A.m1 - pipe_A.m2 * Q_P * (Q_A - K_A) / abs(Q_A - K_A)
        mb, nb = self.solve_equations(k1, k2, b)
        # PA
        b = -b
        mc, nc = self.solve_equations(k1, k2, b)

        # HB
        k1 = -pipe_B.m1 - pipe_B.m2 * abs(Q_B)
        k2 = pipe_A.m1 + pipe_A.m2 * abs(Q_A - K_A)
        b = 1
        mp, np = self.solve_equations(k1, k2, b)
        # QB
        b = -pipe_B.m1 + pipe_B.m2 * (Q_P - K_P) * Q_B / abs(Q_B)
        mq, nq = self.solve_equations(k1, k2, b)
        # PP
        b = -pipe_B.m1 - pipe_B.m2 * Q_B
        mo, no = self.solve_equations(k1, k2, b)
        arr_A = [[ma, mb, mc], [na, nb, nc]]
        arr_B = [[mo, mp, mq], [no, np, nq]]
        return arr_A, arr_B

    # 13
    def update_Jocob(self, As):
        # Internal node
        for pipe in self.pipes:  # Get Xs=X0
            id_pipeA = pipe.id
            id_pipeB = pipe.id
            for j in range(1, pipe.NN):
                k = pipe.count + 3 * j  # k is the index of HP, k+1 is the index of QP
                j_A = j - 1
                j_p = j
                j_B = j + 1
                # Get 2 partial derivative array, comprised of ma,na,mb,nb,mc,nc,mo,no,mp,np,mq,nq
                tup = self.Jocob(id_pipeA, id_pipeB, j_A, j_B, j_p)
                As[k:k + 2, k - 3:k] = tup[0]
                As[k:k + 2, k + 2:k + 5] = tup[1]
                As[k + 2, k + 2] = 1  # DKP /  DK
        # Boundary node
        for node in self.nodes:
            id = node.id
            typeID = node.typeID
            node.start_pipe = id - 1
            node.end_pipe = id
            if node.type == BD.UpperReservoir:  # Upstream reservoir
                r = self.reservoirs[typeID]
                pipe = self.pipes[node.end_pipe]
                k = pipe.count
                As[k][0] = 1  # HP(HP)
                As[k + 1][3] = -1 / (pipe.m1 + pipe.m2 * abs(pipe.Q[1]))  # QP(HB)
                As[k + 1][4] = (pipe.m1 - pipe.m2 * abs(pipe.Q[0])) * (-As[k + 1][3])  # QP(QB)
            elif node.type == BD.LowerReservoir:  # Downstreaam reservoir
                r = self.reservoirs[typeID]
                pipe = self.pipes[node.start_pipe]
                k = pipe.count + pipe.NN * 3
                j_A = pipe.NN - 1
                j_P = pipe.NN
                Q_A = pipe.Q[j_A]
                K_A = pipe.K[j_A]
                Q_P = pipe.QP[j_P]
                As[k][k] = 1  # HP(HP)
                As[k + 1][k - 3] = 1 / (pipe.m1 + pipe.m2 * abs(Q_A - K_A))  # QP(HA)
                As[k + 1][k - 2] = (pipe.m1 - pipe.m2 * Q_P * (Q_A - K_A) /
                                    abs(Q_A - K_A)) * (As[k + 1][k - 3])  # QP(QA)
                As[k + 1][k - 1] = - As[k + 1][k - 2]  # QP(KA)
                # KP(KP)
                As[k + 2][k + 2] = 1
            elif node.type == BD.Series:  # Series node
                id_pipeA = node.start_pipe
                id_pipeB = node.end_pipe
                j_A = self.pipes[id_pipeA].NN - 1
                j_B = 1
                j_P = self.pipes[id_pipeA].NN
                k = self.pipes[id_pipeB].count
                # Get 2 partial derivative array, comprised of ma,na,mb,nb,mp,np,mq,nq
                tup = self.Jocob(id_pipeA, id_pipeB, j_A, j_P, j_B)
                As[k - 3:k - 1, k - 6:k - 3] = tup[0]
                As[k - 3:k - 1, k + 2:k + 5] = tup[1]
                As[k:k + 2, k - 6:k - 3] = tup[0]
                As[k:k + 2, k + 2:k + 5] = tup[1]
            elif node.type == BD.EndBallValve:
                bv = self.ballvalves[typeID]
                CDA = bv.T2CDA(self.T)
                ...
                b = -b
                mc, nc = self.solve_equations(k1, k2, b)
                As[k:k + 2, k - 3:k] = [[ma, mb, mc], [na, nb, nc]]
                # KP(KP)
                As[k + 2][k + 2] = 1
                # CDA
                b2 = -2 * H_P / bv.T2CDA(self.T - self.time_step)  # * bv.dCDA_dt(self.T)
                mr = b2 * k1 / (k1 - k2)
                nr = -b2 / (k1 - k2)
                As[k:k + 2, k + 3] = [mr, nr]
                # CDA(CDA)
    ...
    # 14
    def solve_equations(self, k1, k2, b):
        return -b * k2 / (k1 - k2), b / (k1 - k2)

    # 15
    def write_As(self, x):
        writer = pd.ExcelWriter('As.xlsx')		# 写入Excel文件
        data = pd.DataFrame(self.As)
        data.to_excel(writer, 'As', float_format='%.4f')		# ‘page_1’是写入excel的sheet名
        data = pd.DataFrame(self.Xs)
        data.to_excel(writer, 'Xs', float_format='%.4f')		# ‘page_1’是写入excel的sheet名
        data = pd.DataFrame(x)
        data.to_excel(writer, 'x', float_format='%.4f')		# ‘page_1’是写入excel的sheet名
        writer.save()
        writer.close()
    ...
    def small_signal(self):
        ...
        '''
        x = np.asmatrix(np.zeros((self.N, self.steps)))  # dx
        X_ave = np.asmatrix(np.zeros((self.N, self.steps)))  # X
        ...
        self.T = 0
        # dx(0) = X(1) - X(0)
        x[:, 0] = X_ave[:, 0] - self.Xs[:, 0]

        for k in range(1, self.steps):
            self.T = self.T + self.time_step
            # boundary condition
            bv = self.ballvalves[0]
            x[-1, k - 1] = bv.dCDA(self.T)
            # dx(k) = As * dx(k-1)
            x[:, k] = np.dot(self.As, x[:, k - 1])
            # X(t) = Xs + dx(k)
            X_ave[:, k] = self.Xs[:, k - 1] + x[:, k]

            # update H, Q, K
            self.inv_state_var(X_ave[:, k])  # Update H,Q
            # update As
            self.update_Jocob(self.As)
            # Upate Xs
            self.Xs[:, k] = X_ave[:, k]
    ...
    def leak_pipe(self):
        leak=[]
        for pipe in self.pipes:
            leak.append(sum(pipe.K))
        leakFlow = max(leak) # max sum leak 
        leakPipe = leak.index(leakFlow)  # where the max sum leak is
        leaks = self.pipes[leakPipe].KP # leak series at the leak pipe
        leakPoint=np.where(leaks==np.max(leaks))[0][0]
        return [leakPipe, leakPoint, leakFlow]

    def fx(self, x):
        self.inv_state_var(x)
        self.transient(timeup=False)
        y = np.zeros(x.shape[0])
        self.get_state_var(y)
        return y

    def get_sensor_point(self,points):
        sensor=[]
        for point in points:
            sensor+=[self.pipes[point[0]].count+3*point[1]]
        return sensor

    def get_point(self,point,k):
        return self.pipes[point[0]].count+point[1]*3+k
    
    def point2pipe(self,point):
        for pipe in self.pipes:
            if point< pipe.count+pipe.NN*3:
                node=int(round((point-pipe.count)/3))
                return pipe.id,node
        return pipe.id,pipe.NN

    def get_transfer_matrix(self, omega,n=0,k=0):
        '''
        Moc: in-node, boundary node, update state variable and time
        '''
        for pipe in self.pipes:
            pipe.get_field_matrix(omega)
        for node in self.nodes:
            node.is_transfer_q=False
            node.get_point_matrix(omega)
        self.u = np.zeros((self.n_pipe * 4, self.n_pipe * 4),dtype=complex)
        self.Y = np.zeros(self.n_pipe * 4, dtype=complex)
        pos=0
        for pipe in self.pipes:
            # js for point
            p = pipe.id * 4
            node = pipe.start_node
            typeID=node.typeID
            other_pipe = node.get_other_pipe(pipe)
            if node.type==BD.Branch:
                if not node.is_transfer_q:
                    self.u[pos, p] = -1
                    for i, op in enumerate(other_pipe):
                        self.u[pos + 1 + i, p + 1] = -1
                        # q1=q2+q3
                        if op.start_node is node:
                            # outflow
                            pR = op.id * 4
                            self.u[pos, pR] = -1
                            self.u[pos + 1 + i, pR+1] = 1
                        elif op.end_node is node:
                            # inflow
                            pR = op.id * 4+2
                            self.u[pos, pR] = 1
                            self.u[pos + 1 + i, pR + 1] = 1
                        # H1=H2=H3
                    pos+=node.np                
                    node.is_transfer_q=True
            else:
                
                if node.type==BD.Series:
                    if node.id == other_pipe.js:
                        pR = other_pipe.id * 4
                    else:
                        pR = other_pipe.id * 4 + 2
                    # if the direction of two pipe is inverse
                    self.u[pos,p]=-1
                    self.u[pos,pR]=1
                    self.u[pos+1, p+1] = -1
                    self.u[pos+1, pR+1] = 1
                    ...
                    # inv(F)?
                    self.u[pos, p] = 0
                    self.u[pos, p + 2] = pipe.field_matrix[0, 0]
                    self.u[pos, p + 3] = pipe.field_matrix[0, 1]
                    pos += 1
            ...
            pe = pipe.id * 4+2
            self.u[pos,pe]=-1
            self.u[pos,pe-2]=pipe.field_matrix[0,0]
            self.u[pos, pe-1] = pipe.field_matrix[0, 1]
            self.u[pos+1, pe+1] = -1
            self.u[pos+1,pe-2]=pipe.field_matrix[1,0]
            self.u[pos+1, pe-1] = pipe.field_matrix[1, 1]
            pos+=2
            node = pipe.end_node
            p = pipe.id * 4+2
            if (node.type == BD.UpperReservoir) or (node.type == BD.LowerReservoir):
                ...
                self.u[pos, p] = 0
                self.u[pos, p - 2] = pipe.field_matrix[0, 0]
                self.u[pos, p - 1] = pipe.field_matrix[0, 1]
                pos += 1
            elif node.type==BD.EndValve:
                valve=node.obj
                self.u[pos, p+1] = -1
                self.u[pos, p ] =pipe.H0[pipe.NN]*2/pipe.Q0[pipe.NN]
                if n==0:
                    self.Y[pos] = pipe.H0[pipe.NN] * 2 *valve.amplitude
                else:   
                    self.Y[pos] = pipe.H0[pipe.NN] * 2 * valve.real_f[n+1]/2
                pos += 1    
        x=np.linalg.solve(self.u,self.Y)
        # saveExcel([self.u,self.Y, x],'twovalve')
        return x
    ...
    def plot_frequency_response(self):
        ...
        hf=[]
        qf=[]
        # plt.plot(self.wfs,self.endValves[0].taus)
        # plt.show()
        k=0.2
        for i,w in enumerate(omegas):
            a=self.get_transfer_matrix(w,i,k)
            hf += [a[-1]]
            qf += [a[-2]]
        self.pow_q=np.abs(qf)
        self.pow_h = np.abs(hf)
        self.hifft=np.fft.ifft(hf)+100
        self.qifft=np.fft.ifft(qf)+0.314
        magh = np.load('transfer_matrix_1_magh.npy')  # 加载文件
        magq = np.load('transfer_matrix_1_magq.npy')  # 加载文件
        freq = np.fft.fftfreq(self.steps, self.time_step)*2*np.pi
        ax=plt.subplot(211)
        plt.plot(omegas[omegas > 0], self.pow_q[omegas > 0], '--', label='2|qr|/Q0')
        plt.plot(freq[freq > 0], magq[freq > 0], label='moc_q')
        self.omegas=omegas
        plt.legend()
        ax = plt.subplot(212)
        plt.plot(omegas[omegas > 0], self.pow_h[omegas > 0], '-', label='2|hr|/H0')
        plt.plot(freq[freq > 0], magh[freq > 0], label='moc_h')
        # plt.plot(self.t[1:], self.hifft, '--', label='2|qr|/Q0')
        plt.xlabel('wr=w/wn')
        plt.ylabel('hr,qr')
        # plt.axis([0,7,0,0.85])
        plt.legend()
    ...
    def plot_frequency_diagram(self):
        omegas=np.linspace(0.01,self.wn*7,700)
        hf=[]
        qf=[]
        for i in omegas:

            a=self.get_transfer_matrix(i)
            hf += [abs(a[-1] * 2 / 100)]
            qf += [abs(a[-2]) * 2 / 0.314]
        plt.plot(omegas/self.wn,qf,'--',label='2|qr|/Q0')
        plt.plot(omegas/self.wn,hf,'-',label='2|hr|/H0')
        plt.xlabel('wr=w/wn')
        plt.ylabel('hr,qr')
        plt.axis([0,7,0,0.85])
        plt.legend()
    ...
    def remove_leaks(self):
        for pipe in self.pipes:
            pipe.leaks=[]
            pipe.cda = np.zeros(pipe.NN + 1)  # Leak of current time step
        
    def add_leak(self,leak):
        pipe = self.pipes[leak.pipe]
        pipe.leaks.append(leak)
        pipe.leakflow=np.zeros((len(pipe.leaks),self.steps))
